...
class _RandomPracticeScreenState extends State<RandomPracticeScreen>
  ...
  void _startPrep() {
    ...
    setState(() { _phase = _Phase.prep; _secsLeft = kPrepSeconds; });
    _startTimer();
  }

  void _startSpeaking() {
    HapticFeedback.heavyImpact();
    setState(() { _phase = _Phase.speaking; _secsLeft = kSpeakSeconds; });
    _startTimer();
  }
  ...
  void _stop() {
    _timer?.cancel();
    setState(() { _phase = _Phase.idle; _secsLeft = kPrepSeconds; });
  }
  ...
  Color _timerColor(bool isDark) {
    if (_phase == _Phase.prep)
      return _secsLeft <= 10 ? const Color(0xFFE65100) : const Color(0xFFFFB300);
    if (_phase == _Phase.speaking)
      return _secsLeft <= 20 ? const Color(0xFFC62828) : const Color(0xFF2E7D32);
  ...
  Widget build(BuildContext context) {
    ...
    final color     = _card != null ? _catColor(_card!.category) : const Color(0xFF1565C0);
  ...
  Widget _buildError(bool isDark) => Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
    const Icon(Icons.error_outline_rounded, size: 48, color: Color(0xFFC62828)),
    const SizedBox(height: 12),
    Text('Could not load cards', style: TextStyle(fontSize: 14,
        color: isDark ? const Color(0xFF8899AA) : const Color(0xFF555577))),
    const SizedBox(height: 16),
    ElevatedButton.icon(onPressed: _loadRandom,
        icon: const Icon(Icons.refresh_rounded), label: const Text('Retry')),
  ]));

  Widget _buildPhaseBanner(bool isDark) {
    final configs = {
      _Phase.idle:     _BannerConfig(icon: Icons.lightbulb_outline_rounded,
  ...
  Widget _buildTimerRing(bool isDark) {
    final max      = _phase == _Phase.prep ? kPrepSeconds.toDouble() : kSpeakSeconds.toDouble();
      ...
      child: SizedBox(width: 160, height: 160,
        ...
        if (_phase == _Phase.idle)
          Column(mainAxisSize: MainAxisSize.min, children: [
            Icon(Icons.touch_app_rounded, size: 32,
                color: isDark ? const Color(0xFF4DB6FF) : const Color(0xFF1565C0)),
            const SizedBox(height: 4),
            Text('Ready?', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700,
                color: isDark ? const Color(0xFF4DB6FF) : const Color(0xFF1565C0))),
          ])
        else if (_phase == _Phase.finished)
          const Column(mainAxisSize: MainAxisSize.min, children: [
            Icon(Icons.check_circle_rounded, size: 40, color: Color(0xFF2E7D32)),
            SizedBox(height: 4),
          ...
          Column(mainAxisSize: MainAxisSize.min, children: [
            Text(_fmt(_secsLeft), style: TextStyle(fontSize: 34,
                fontWeight: FontWeight.bold, color: tc)),
            const SizedBox(height: 2),
            Text(_phase == _Phase.prep ? 'PREP' : 'SPEAK',
                style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700,
                    letterSpacing: 2, color: tc.withOpacity(0.7))),
          ]),
      ])),
    );
  }
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: isDark ? [] : [BoxShadow(
              color: color.withOpacity(0.10), blurRadius: 18, offset: const Offset(0, 5))],
        ),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            const Spacer(),
            Text('Card #${card.id}', style: TextStyle(fontSize: 11,
                color: isDark ? const Color(0xFF557799) : const Color(0xFF888899))),
          ]),
          const SizedBox(height: 14),
          Text(card.topic, style: TextStyle(fontSize: 17, fontWeight: FontWeight.bold,
              height: 1.38,
              color: isDark ? const Color(0xFFE8EAF0) : const Color(0xFF1A1A2E))),
          const SizedBox(height: 14),
          Text('You should say:', style: TextStyle(fontSize: 12.5,
  ...
  Widget _buildMainActions(bool isDark, Color color) {
    switch (_phase) {
      case _Phase.idle:
        return _PrimaryBtn(label: 'Start Preparation (1 min)',
            icon: Icons.play_arrow_rounded,
            color: const Color(0xFF1565C0), onTap: _startPrep);
      case _Phase.prep:
        return Column(children: [
          _PrimaryBtn(label: 'Start Speaking Now',
              icon: Icons.mic_rounded,
              color: const Color(0xFF2E7D32), onTap: _startSpeaking),
          const SizedBox(height: 10),
          _SecondaryBtn(label: 'Stop', icon: Icons.stop_rounded,
              isDark: isDark, onTap: _stop),
        ]);
      case _Phase.speaking:
        return _SecondaryBtn(label: 'Stop Speaking',
            icon: Icons.stop_rounded, isDark: isDark, onTap: _stop);
      case _Phase.finished:
        return _PrimaryBtn(label: 'Next Card',
            icon: Icons.arrow_forward_rounded,
            color: const Color(0xFF6A1B9A), onTap: _nextCard);
    }
  }

  Widget _buildSecondaryActions(bool isDark) {
    return Row(children: [
      Expanded(child: _SecondaryBtn(label: 'Skip Card',
          icon: Icons.skip_next_rounded, isDark: isDark, onTap: _nextCard)),
      const SizedBox(width: 12),
      Expanded(child: _SecondaryBtn(label: 'View Answer',
          icon: Icons.visibility_outlined, isDark: isDark,
...
class _BannerConfig {
  ...
  final String   label;
  final Color    bg, fg;
  const _BannerConfig({required this.icon, required this.label,
  ...
  Widget build(BuildContext context) => SizedBox(width: double.infinity,
    child: ElevatedButton.icon(
      ...
      style: ElevatedButton.styleFrom(
        ...
        elevation: 3, shadowColor: color.withOpacity(0.4),
      ),
    ));
}
...
class _SecondaryBtn extends StatelessWidget {
  ...
  const _SecondaryBtn({required this.label, required this.icon,
      required this.isDark, required this.onTap});
  @override
  Widget build(BuildContext context) => SizedBox(width: double.infinity,
    child: OutlinedButton.icon(
      onPressed: onTap, icon: Icon(icon, size: 17), label: Text(label),
      style: OutlinedButton.styleFrom(
        foregroundColor: isDark ? const Color(0xFF8899AA) : const Color(0xFF555577),
        side: BorderSide(color: isDark ? const Color(0xFF2A3E55) : const Color(0xFFDDDDEE)),
